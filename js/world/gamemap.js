...
GameMap.prototype = Object.create(Map2D.prototype);
...
/**
 * Basically a wrapper for neighboursOf that cuts off the edges
 * @param origin
 * @param size
 * @param includeOrigin
 * @returns {Array.<T>|*}
 */
GameMap.prototype.getRadius = function (origin, size, includeOrigin) {

    var fields = this.neighboursOf(origin, size).filter(function (field) {
        var pos1 = origin.position;
        var pos2 = field.position;
        var dist = pos2.distanceTo(pos1, Position.distances.MANHATTAN);
        return dist <= size;
    });
    if (includeOrigin) fields.push(origin);
